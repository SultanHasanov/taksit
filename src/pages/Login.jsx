...
import { signIn, getUserProfile } from '../firebase/auth';
import { toAuthEmail } from '../lib/credentials';

const DEMO = {
  superadmin: { email: 'super@taksit.ru', password: 'demo1234' },
...
export default function Login() {
  ...
  const handleSubmit = async ({ email, password }) => {
    setLoading(true);
    try {
      const cred = await signIn(toAuthEmail(email), password);
      const profile = await getUserProfile(cred.user.uid);

      if (!profile?.role) {
        message.error('Профиль не найден. Обратитесь к супер-админу.');
        return;
      }

      nav(HOME_BY_ROLE[profile.role] ?? '/', { replace: true });
    } catch (e) {
      if (
        ...
        message.error(`Не удалось войти (${e?.code ?? 'unknown-error'})`);
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
        ...
        >
          <div
            style={{
              width: 21,
              height: 21,
              background: 'linear-gradient(150deg,#E6CD8C,#8C6F30)',
              transform: 'rotate(45deg)',
              borderRadius: 4,
              boxShadow: '0 0 14px rgba(203,164,90,.6)',
            }}
          />
        </div>
        <div>
      ...
      >
        <Form.Item label="Логин" name="email" rules={[{ required: true, message: 'Введите логин' }]}>
          <Input size="large" placeholder="логин или email" />
        </Form.Item>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '24px 0' }}>
        ...
        <span className="faint" style={{ fontSize: 10.5, letterSpacing: '.1em' }}>ЗАЩИЩЕННЫЙ ВХОД</span>
        <span style={{ flex: 1, height: 1, background: 'var(--line)' }} />
      </div>
      <div className="faint" style={{ textAlign: 'center', fontSize: 11, lineHeight: 1.7 }}>
        Банковское шифрование · Вход по биометрии
        <br />
        Лицензированный оператор рассрочки
      </div>
    </div>
  );
}
